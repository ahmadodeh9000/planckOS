...
static void fat_set_cluster(uint32_t cluster, uint32_t value) {
    ...
    // Modify the entry
    uint32_t *fat = (uint32_t *)buffer;
    fat[fat_index] = value & 0x0FFFFFFF;

    // Write it back
    ata_write(fat_sector, 1, buffer);
}

// Find the first free cluster (FAT entry = 0 means free)
static uint32_t fat_find_free_cluster() {
...
static void to_83(const char *filename, char *out) {
    ...
    if (dot == -1) {
        ...
        for (int i = 0; i < dot && i < 8; i++)
            out[i] = filename[i];
        for (int i = 0; filename[dot+1+i] && i < 3; i++)
            out[8+i] = filename[dot+1+i];
    }
}

// Find a directory entry by filename, returns sector LBA and index
// Returns 1 if found, 0 if not
static int fat_find_entry(const char *filename, uint32_t *out_lba,
                           uint32_t *out_index, uint8_t *dir_buf) {
    char name83[11];
